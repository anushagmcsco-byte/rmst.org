...
export default function Header({
  ...
  const [mobileResourcesOpen, setMobileResourcesOpen] = useState(
    ['compliance', 'transparency', 'stories', 'gallery', 'events', 'media'].includes(activePage)
  );
  const [scrolled, setScrolled] = useState(false);

  useEffect(() => {
    if (['compliance', 'transparency', 'stories', 'gallery', 'events', 'media'].includes(activePage)) {
                      ...
                      <div key={item.id} className="space-y-1">
                        <button
                          ...
                          className={`w-full text-left px-3 py-2.5 rounded-xl font-display font-semibold text-sm transition-colors cursor-pointer flex items-center justify-between ${
                            mobileResourcesOpen || ['compliance', 'transparency', 'stories', 'gallery', 'events', 'media'].includes(activePage)
