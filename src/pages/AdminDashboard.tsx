...
const MEDIA_ASSETS_COLLECTION = [
  { title: 'Mallappa Gowda Solar Harvest', tags: ['Agriculture', 'Haveri'], type: 'Image', size: '2.4 MB', url: 'https://images.unsplash.com/photo-1592417817098-8f3d6eb19675?auto=format&fit=crop&q=80&w=300' },
  { title: 'Savanur School Girls Python Camp', tags: ['Education', 'STEM'], type: 'Image', size: '1.8 MB', url: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&q=80&w=300' },
  { title: 'Kundgol Soil Humic Diagnostics Lab', tags: ['Research', 'Water'], type: 'Image', size: '3.1 MB', url: 'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&q=80&w=300' },
  { title: 'Women Dairy Cold Chain Installation', tags: ['Dairy', 'Empowerment'], type: 'Image', size: '4.2 MB', url: 'https://images.unsplash.com/photo-1589156280159-27698a70f29e?auto=format&fit=crop&q=80&w=300' }
];
...
export default function AdminDashboard({ highContrast, setActivePage, seoConfig, setSeoConfig }: AdminDashboardProps) {
  ...
  const [currentAdminTab, setCurrentAdminTab] = useState<'overview' | 'cms' | 'blogs' | 'events' | 'gallery' | 'seo' | 'programs' | 'crm' | 'volunteers' | 'workflow' | 'aistudio' | 'integrations' | 'system'>('overview');
  ...
  const [galleryList, setGalleryList] = useState<any[]>(() => {
    ...
    return MEDIA_ASSETS_COLLECTION;
  });
  ...
  const deleteCmsPage = (id: string) => {
    if (confirm('Are you sure you want to delete this content item from public rendering list?')) {
      setCmsList(cmsList.filter(p => p.id !== id));
    }
  };
  ...
  const deleteWorkflow = (id: string) => {
    if (confirm('Delete this automated integration script?')) {
      setWorkflowAutomations(workflowAutomations.filter(wf => wf.id !== id));
    }
  };
                                ...
                                <button
                                  onClick={() => {
                                    if (confirm('Delete this blog post?')) {
                                      setBlogsList(blogsList.filter(b => b.id !== blog.id));
                                    }
                                  }}
                                  className="px-2 py-1 bg-rose-50 text-rose-600 rounded border border-rose-200 hover:bg-rose-100 cursor-pointer text-[10px]"
                                >
                ...
                <div className="p-6 bg-white border border-slate-200 rounded-3xl text-left space-y-6 animate-fade-in font-sans">
                  ...
                  {(isCreatingGallery || editingGallery) ? (
                        ...
                        <button
                          onClick={() => {
                            if (isCreatingGallery) {
                              ...
                              setGalleryList([newAsset, ...galleryList]);
                              setIsCreatingGallery(false);
                            } else {
                              const updated = galleryList.map((g, idx) => idx === editingGallery._index ? editingGallery : g);
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 font-sans">
                      {galleryList.map((asset, index) => (
                        <div key={index} className="rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm flex flex-col justify-between group relative animate-fade-in">
                          <div>
                          ...
                          <div className="p-4 pt-0 flex justify-between items-center border-t border-slate-100 mt-2 font-mono text-[10px]">
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={() => {
                                  setEditingGallery({ ...asset, _index: index });
                                  setIsCreatingGallery(false);
                                }}
                              ...
                              <button
                                onClick={() => {
                                  if (confirm('Delete this gallery asset?')) {
              ...
              {currentAdminTab === 'seo' && (
                <div className="p-6 bg-white border border-slate-200 rounded-3xl text-left space-y-6 animate-fade-in font-sans">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-4 border-b border-slate-100">
                    ...
                    <button
                      onClick={() => {
                        if (confirm('Are you sure you want to restore all page SEO entries to trust defaults?')) {
                  ...
                  <div className="space-y-6 max-h-[700px] overflow-y-auto pr-2">
                    {Object.keys(seoConfig || {}).map((pageKey) => {
                      const details = (seoConfig || {})[pageKey];
                      return (
